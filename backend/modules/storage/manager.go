...
func MountPool(pool Pool) error {
	...
	base := filepath.Join("/mnt/c0r3nex/pools", pool.Name)
	dataMounts := []string{}
...
func EnsurePoolDirs(poolName string) error {
	base := filepath.Join("/mnt/c0r3nex/pools", poolName)
	sub := []string{
		base,
...
func ResolveSharePath(pool, name, explicit string) string {
	...
	if pool != "" {
		...
		return filepath.Join("/mnt/c0r3nex/pools", pool, "shares", name)
	}
	return filepath.Join("/var/lib/c0r3nex/data/shares", name)
}
...
// RebuildSambaConfig schreibt alle SMB-Shares in /etc/samba/c0r3nex-shares.conf neu und reloaded smbd.
func RebuildSambaConfig() error {
	...
	confPath := "/etc/samba/c0r3nex-shares.conf"
...
func ensurePoolExists(name string) error {
	...
	data, err := os.ReadFile("/var/lib/c0r3nex/pools.json")
	if err != nil {
...
func loadNASUsernames() []string {
	path := "/var/lib/c0r3nex/nas_users.json"
...
func ensureSmbInclude() {
	...
	if strings.Contains(string(data), "include = /etc/samba/c0r3nex-shares.conf") {
		return
	}
	...
	defer f.Close()
	_, _ = f.WriteString("\n# C0R3NEX includes\ninclude = /etc/samba/c0r3nex-shares.conf\n")
}
